...
double eta_NLL() { return                        // See (4.17) in AMY-II
                   pow(Temp,3.)*eta1[Nf]/(
                   pow(g,4)*log(mSt[Nf]/sqrt(g*g*mD2/(4.*M_PI)))    );}

void   eval_T(double,double); // eta(T/Lam)
void   eval_g(double,double); // eta(g) 
int   points;
void   rate_E(double,double,double); // scattering rate (Emin,Emax,Temp)

/*--------------------------------------------------------------------*/

int main(int argc, char **argv) {    // explore... T, alpha  dependence
...
{ alf_run=1; g = 1.; double res1, res2, res3, res4;
       ...
       if (!HTL) sprintf(fname,//
           "outdata/etaT3_kappa%.2f_nf%d_running.dat", kappa, Nf );
  else if  (HTL) sprintf(fname,//
          "out/data/etaT3_HTL_nf%d_running.dat", Nf );

  file = fopen(fname,"w+");

       if (!HTL) fprintf(file,//
           "# screening: M_eff, kappa = %.3f\n", kappa );
  ...
  for(int i=0; i<points; i++) {
    ...
    /*Temp = Tmax*pow(10., -(points -1 - i)*
     *       ( log(Tmax/Tmin)/log(10.))/((double) points - 1));*/
    printf("  :  %-1.4f  :", Temp/lambda);
    fprintf(file, "%.4f", Temp/lambda);
    J = .0;  res4 = eta(); // t^\star = J*T2
    J = .5;  res3 = eta(); //
    J = 2.;  res2 = eta(); //
    J = 1.;  res1 = eta(); //
    printf("  %1.2e  :\n",res1);
...
{ alf_run=0; Temp = 1.; double res1, res2, res3, res4;
       ...
       if (!HTL) sprintf(fname, "out/data/etaT3_kappa%.2f_nf%d_fixed.dat",
                                                                kappa,Nf);
  ...
  printf("  ----------------------------------------------------------\n");
  for(int i=0; i<points; i++) {
    g = gmax*pow(10., -(points -1 - i)*( log(gmax/gmin)/log(10.))/
                                                    ((double) points - 1));
...
{ alf_run=1; Temp = TT; double res1, res2, res3, res4; double Ene; 
  //g=sqrt(4.*M_PI*1.);
       ...
       if (!HTL) sprintf(fname,//
            "out/data/RT_T%.1f_kappa%.2f_nf%d_running-g.dat",Temp,kappa,Nf);
  else if  (HTL) sprintf(fname,//
            "out/data/RT_T%.1f_HTL_nf%d_running-g.dat", Temp, Nf);
  /*if (!HTL) sprintf(fname, "out/data/RT_T%.1f_kappa%.2f_nf%d_fixed-g.dat",
   *                                                     Temp, kappa, Nf  );*/
  /*else if  (HTL) sprintf(fname, "out/data/RT_T%.1f_HTL_nf%d_fixed-g.dat",
   *                                                            Temp, Nf  );*/
  ...
  for(int i=0; i<points; i++) {
    ...
    Ene = Emax*pow(10., -(points -1 - i)*( log(Emax/Emin)/log(10.))/
                                                     ((double) points - 1));
